...
public class MinHeap<MovieRating extends Comparable<MovieRating>> {
	private ArrayList<MovieRating> Heap;
	private int n; // Number of things in heap

	/** Constructor for initially empty heap */
	public MinHeap() {
		Heap = new ArrayList<MovieRating>();
		n = 0;
	}

	public MinHeap(MovieRating[] h) {
		Heap = new ArrayList<MovieRating>(Arrays.asList(h));
		n = h.length;
		buildheap();
	}
	
	public MovieRating get(int pos) {
		assert (pos >= 0) && (pos < n): ("Position is not valid");
			return Heap.get(pos);
	}
	...
	public void insert(MovieRating val) {
