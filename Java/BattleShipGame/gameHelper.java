...
public class GameHelper {
	...
	public ArrayList<String> placeShip(int shipSize){
		...
		while(!success & attempts++ < 200){				//main search loop (32)	
			...
			System.out.print(" try " + location);		//DEBUGGING STATEMENT
			...
			while(success && x < shipSize){				//look for adjacent unused spots
				if(grid[location] == 0){			//if not already used
					...
					if(x>0 && (location % gridLength == 0)){	// out of bounds - 'right edge'
						...
						System.out.print("used " + location);	//DEBUGGING STATEMENT	
						success = false;			//failure
					}	
				}
			}						//end while

		int x = 0;						//turn locations into alpha coords
		int row = 0;
		int column = 0;
		System.out.println("\n");
		while (x < shipSize){
			...
			x++;
			System.out.print(" coord "+ x + " = " + alphaCells.get(x-1));	//DEBUGGING STATEMENT
		}
		System.out.println("\n");						//DEBUGGING STATEMENT
		return alphaCells;
